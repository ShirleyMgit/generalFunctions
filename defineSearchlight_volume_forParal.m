function [L,exclMask] = defineSearchlight_volume_forParal(subN,subDir,ROIMask,Mask,varargin)

%subDir = [data_path,'sub',num2str(subN)];
% cd(subDir);
% disp(pwd);
...
%% 1. Checking inputs
Opt.sphere    = [30 160]; % 30mm radius volumetric sphere, 160 voxels (the default)
...
Vin(2) = ROIMask; 
Vo     = Vin(1); 
inclMaskName = ['inclMask',num2str(subN),'.nii']; 
Vo.fname =  inclMaskName;
...
if (Opt.writeMask) 
    Vo     = Vin(1); 
    Vo.fname =  ['exclMask',num2str(subN),'.nii']; 
