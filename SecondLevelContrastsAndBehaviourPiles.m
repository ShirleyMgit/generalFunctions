function SecondLevelContrastsAndBehaviourPiles(numr,nameF)
...
contrastDir = fullfile(data_path,nameF,'searchLightResults','contrast','wholeBrainMcorrect','DistRegCleaned_onlyMotionCSF_newMaskBs1','s6DistRegCleaned_onlyMotionCSF_newMaskBs1nSub28');
...
contrasts = {'proHexStrNonCol','proBothStronCol'};
...
for contrast_loop=1:length(contrasts)
    ...
    for subj_loop = 1:nSub
        con_file = cellstr(spm_select('FPList',fullfile(contrastDir,['sub',num2str(vnP(subj_loop))]),[contrasts{contrast_loop},'.nii']));
