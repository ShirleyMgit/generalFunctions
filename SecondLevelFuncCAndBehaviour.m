...
name_analysis = [name_analysis,nSub',num2str(nSub)];
...
%dmean:
vratCorMHall = vratCorMHall - mean(vratCorMHall);
vratCorMCall = vratCorMCall - mean(vratCorMCall);
...
%conNum = [4:28]
contrasts = {'rest','hex','hexMinusRest','cluster','clusterMinusRest','hexAndCluster','hexMinusCluster'};%{'Hex','cluster','hexAndCluster','hexMinusCluster'};
...
for contrast_loop=conStart:conEnd
    ...
    for subj_loop = 1:nSub
        if conNum(contrast_loop)<10
            con_file = cellstr(spm_select('FPList', [data_path,'sub',num2str(vnP(subj_loop)),'/stats/',typeDir,'/'], ['^s6con_000',num2str(contrast_loop),'.nii$']));
        else
            con_file = cellstr(spm_select('FPList', [data_path,'sub',num2str(vnP(subj_loop)),'/stats/',typeDir,'/'], ['^s6con_00',num2str(contrast_loop),'.nii$']));
        end
        disp('now display contrast files found:')
        disp([data_path,'sub',num2str(vnP(subj_loop)),'/stats/',typeDir,'/'])
        disp(con_file);
