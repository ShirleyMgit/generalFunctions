...
nameDirO = fullfile(data_path,dirT,'PileSimSLnative',['L',sizeL,areaM],['sub',num2str(vnpar(nsb))]);
mkdir(nameDirO);
% the names of the images created by the searchlight:
outFiles = cell(1,nFiles);
for f=1:nFiles
    outFiles{f} = fullfile(nameDirO,['L',sizeL,num2str(f),'.nii']);
end
...
pathSL= fullfile(data_path,'NativeSearchLightDefinitions',areaM,dirL,['s',num2str(vnpar(nsb))],nameSL);
load(pathSL);
runSearchlight_ShirleyRunNtempDdifName(L,inFiles,outFiles,@pileSimWithinMap,nsb,runN,Datafile,'optionalParams',{SPM});
