function H = BetaCor_Pile_LooFixNormMElementsNoNanInf_RSAtoolboxCheckF(RowData,SPM)
...
wbPile = 1:3:12;% distance betas
v(wbPile) = 1;% take the distance regressor
zeroBetas = 3 + 3 +7;
bn0 =  [zeros(1,5) v zeros(1,zeroBetas)];%[zeros(1,5) v zeros(1,5)];%zeros in the end - betas for 5 maps,the catchX2, the start and the movement regressors + ventricles regressors
bn = [bn0 bn0 bn0 bn0];
bnb = [bn 0 0 0 0];
nwb = length(bn);
beta_num = sum(bn);
...
%disp(lvvoxels);put into comment 5.10
betaAll1 = reshape(betaAll0,[4,4,lvvoxels]);
betaAllLooAv = zeros(4,4,lvvoxels);
