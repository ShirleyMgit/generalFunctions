function makePileBetaContrast(sb,isPC,dirT,nameAna,nSm,Lsize)
...
mROI_dat = zeros(91,109,91,nFiles);

nameAnaO = ['s',num2str(nSm),nameAna,'nSub',num2str(nSub)];%,'45out'];

nameDir = fullfile(data_path,dirT,['searchLightAllL',num2str(Lsize)],['sub',num2str(vnpar(sb))],areaM);
...
nameDirAll = fullfile(data_path,dirT,'searchLightResults','contrast',areaM,nameAna);
%nameDirAll = [data_path,dirT,'All/searchLightResults/',areaM,'/'];
if ~exist(nameDirAll)
    mkdir(nameDirAll);
end

savePileBetaMapsContrast(vnpar(sb),mROI_dat,nameDirAll,ROI,nameAnaO);
